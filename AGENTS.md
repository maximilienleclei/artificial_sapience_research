...
20. A bounded run is a safety slice, not evidence of convergence. Do not treat "it hit the time cap" as "it converged." Convergence must be judged from saved optimization curves across repeated bounded slices.
21. When the goal is convergence, run repeated bounded slices that append metrics/artifacts, inspect whether the objective is still improving meaningfully, and continue until the curve has genuinely flattened or the user chooses to stop.
22. Background runs are allowed when they improve responsiveness, but they must be fully trackable: use a unique run name, a dedicated output directory, and a machine-readable status file that records command, start time, stop deadline, and output paths.
